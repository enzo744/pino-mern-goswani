...
const LikeCount = ({props}) => {
    ...
    const { data:blogLikeCount, loading, error } = 
        useFetch(`${getEnv('VITE_API_BASE_URL')}/blog-like/get-like/${props.blogid}/${user && user.isLoggedIn ? user.user._id : ''}`,
            {
              method: "get",
              credentials: "include",
            })
            ...
            const handleLike = async () => {
                try {
                    if(!user.isLoggedIn) {
                        return showToast('error', 'Devi accedere per fare il like!')
                    }
  ...
  return (
    <button onClick={handleLike} type='button' className='flex items-center justify-between gap-1'>
